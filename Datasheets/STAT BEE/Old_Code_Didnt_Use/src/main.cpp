...
void handleGameStart(){
  ...
  unsigned long d = doc["durationMs"] | 10000;
  gameDuration = d;
  for (int i=0;i<10;i++){ 
    pressOrder[i] = -1; 
    recorded[i] = false;
...
void handleEvents() {
  ...
  server.sendContent(": Welcome to Quiz Buzzer SSE\n\n");
  server.client().stop();
}
...
void sendSSEEvent(const char* event, const char* data) {
  ...
  // In a real implementation, you would maintain a list of connected clients
  // and send the event to each one. For simplicity, we'll just log it here.
